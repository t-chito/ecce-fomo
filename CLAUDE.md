...
テンプレートやスタイルの変更、リファクタなど、repo の作りに手を入れる保守作業は、main に直接 push せず PR を切ってユーザーのレビューを経る。ここで言う「修正」は repo の構造やコードをいじる作業を指す。`/news-publish` による日々の発行は「修正」ではなく正規の発行動作であり、main へ直接 push する（この節の対象外）。
